...
Compiler.prototype = {
  ...
  visitBlock: function(block){
    ...
    for (var i = 0; i < len; ++i) {
      ...
      if (block.nodes[i+1] && block.nodes[i].isText && block.nodes[i+1].isText)
        // Fix for derby. Separate text nodes with whitespace
        //this.buffer('\\n');
        this.buffer(' ');
    }
  },
  ...
  visitTag: function(tag){
    ...
    if ((~selfClosing.indexOf(name) || tag.selfClosing) && !this.xml) {
      ...
      if (name[name.length - 1] != ':') // Derby tags support
        this.buffer('</' + name + '>');
    }
    this.indents--;
  },
  ...
  visitCode: function(code){
    ...
    // Make derby else block inherit {{}} or {} from #if
    var elseEnd = ''
    if(code.val.slice(0, 6) == '\'{else'
      || code.val.slice(0, 7) == '\'{{else') {
      var i = this.buf.length - 1;
      while(i >= 0) {
        if(this.buf[i] == 'buf.push(\'{/}\');') {
          elseEnd = '{/}';
          var captures = /^'\{+([^\}]*)\}+'$/.exec(code.val)
          code.val = '\'{' + (captures[1] || '' ) + '}\'';
          break;
        } else if(this.buf[i] == 'buf.push(\'{{/}}\');') {
          elseEnd = '{{/}}';
          var captures = /^'\{+([^\}]*)\}+'$/.exec(code.val)
          code.val = '\'{{' + (captures[1] || '' ) + '}}\'';
          break;
    ...
    if (code.block) {
      ...
      // Derby support
      if(code.val.slice(0, 2) == '\'{') {
        if(elseEnd) {
          this.buffer(elseEnd);
        } else if(code.val.slice(0, 3) == '\'{{') {
          this.buffer('{{/}}');
        } else {
          this.buffer('{/}');
        }
